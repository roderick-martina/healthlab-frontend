<template>
  <div class="border rounded-lg bg-white">
    ...
    <div class="mx-6">
      ...
      <div class="flex flex-col mt-2">
        <label class="font-bold">(Research) project</label>
        <div class="py-2 mb-1">
          <select v-model="filter.research_project" class="h-8 border bg-white w-full focus:outline-none">
            <option selected value>Choose education</option>
            <option
              v-for="(option,index) in researchProjectOptions"
              :key="index"
              :value="option"
            >{{option}}</option>
          </select>
...
export default {
  ...
  props: ['activityOptions', 'ethnicityOptions','researchProjectOptions'],
  data() {
    return {
      filter: {
        gender: null,
  ...
  methods: {
    ...
    getParams: function() {
      let params = {
        age: this.age,
        gender: this.gender,
        research_project: this.filter.research_project,
