<template>
  <div class="border rounded-lg bg-white">
    ...
    <div class="mx-6">
      ...
      <div class="border-b"></div>
      <div class="flex flex-col mt-2">
        <label class="font-bold">Bmi</label>
        <div class="py-2 mb-1">
          <input
            class="h-8 w-16 border rounded pl-5 focus:outline-none"
            placeholder="10"
            v-model="filter.minBmi"
          >
          -
          <input
            class="h-8 w-16 border rounded pl-5 focus:outline-none"
            placeholder="31"
            v-model="filter.maxBmi"
            :disabled="filter.minBmi == ''"
          >
        </div>
      </div>
      <div class="border-b"></div>
      <div class="flex flex-col mt-2">
        <label class="font-bold">Ethnicity</label>
        <div class="py-2 mb-1">
          <select v-model="filter.ethnic" class="h-8 border bg-white w-full focus:outline-none">
            <option selected value>Choose ethnicity</option>
            <option
              v-for="(option,index) in ethnicityOptions"
              :key="index"
              :value="option"
            >{{option}}</option>
          </select>
...
export default {
  ...
  computed: {
    ...
    bmi: function() {
      if (this.filter.maxBmi == null || this.filter.maxBmi == '') {
        return this.filter.minBmi
      } else {
        return `${this.filter.minBmi}-${this.filter.maxBmi}`
      }
    }
  },
  methods: {
